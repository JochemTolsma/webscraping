...
library(knitr)
opts_chunk$set(tidy.opts=list(width.cutoff=100),tidy=TRUE, eval = FALSE, warning = FALSE, message = FALSE,comment = "#>", cache=TRUE, class.source=c("test"), class.output=c("test2"), cache.lazy = FALSE)
options(width = 100) 
rgl::setupKnitr()
```

```{r klippy, echo=FALSE, include=TRUE, message=FALSE, eval=TRUE}
...
```{r customfunctions, results='hide', eval = TRUE}
...
`r colorize("How incredibly annoying, when trying to do the final knit for the workshop, Leiden university decided to do maintance work on their website on Saturday 22 and Sunday 23 June!!!!!!!", "red")`
